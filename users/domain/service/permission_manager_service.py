...
class PermissionManagerService(BaseService):
    """
    Manages group-level permissions and user-business permission queries.
    """
    ...
    def set_group_permissions(group: Group, permissions: dict[str, dict[str, bool]]):
        for group_permission, is_marked in permissions.items():
            ...
            PermissionManagerService.set_group_permission(
                group=group, permission=permission, action=is_marked
            )
    ...
    def get_user_businesses_permissions(user: User, json_format=True):
        user_business_perms = UserBusinessPermission.objects.raw(
            "SELECT id, business_key_id, permission_id, active FROM permissions_userbusinesspermission WHERE user_key_id = %s",
            [user.id],
        )
        ...
        for ubp in user_business_perms:
            ...
            nombre_permiso = (
                ubp.permission_id
                if json_format
                else Permission.objects.get(id=ubp.permission_id).name
            )
    ...
    def get_user_groups(user: User, json_format=True):
        user_groups = BusinessMembership.objects.raw(
            "SELECT id, business_key_id, group_key_id FROM permissions_groupbusinesspermission WHERE user_key_id = %s",
            [user.id],
        )
        groups_dict = {}
        ...
        for group in user_groups:
            ...
            for group in user_groups:
                nombre_grupo = (
                    group.group_key_id
                    if json_format
                    else Group.objects.get(id=group.group_key_id).name
                )
