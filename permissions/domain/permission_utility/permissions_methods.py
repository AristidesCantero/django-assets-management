...
def user_is_superadmin(user_id:str):
    return User.objects.filter(pk=user_id,is_superuser=True,active=True)
    

def user_has_permission(user_id:str, business_id:str, permission_name):
...
def type_of_user_access(user_id: str | User,business_id: str,permission:str) -> str | None:
    ...
    if not isinstance(user_id,(User,int,str)):
      raise TypeError(f"user value expected to be User, str or int, recieved {type(user_id)}")

    if (isinstance(user_id,User) and user_id.is_superuser) or user_is_superadmin(user_id):
      return 'superuser'
      
    
    user_has_access = user_has_permission(user_id, business_id, permission)
    if user_has_access:
      return 'regular'
...
def get_permission_codename(view,request):
    ...
    if not method in method_to_action.keys():
      return ""
    
    return f"{action}_{view.serializer_class}"   
